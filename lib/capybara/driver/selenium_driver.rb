...
class Capybara::Driver::Selenium < Capybara::Driver::Base
  class Node < Capybara::Node
    ...
    def set(value)
      if tag_name == 'textarea' or (tag_name == 'input' and %w(text Text TEXT password Password PASSWORD hidden Hidden HIDDEN file File FILE).include?(type))
        node.clear
    ...
    def select(option)
      option_node = node.find_element(:xpath, ".//option[normalize-space(text())='#{option}']") || 
